...
export const CatalogProductsHome = () => {
    const [products, setProducts] = useState<Product[]>([]);
    const [pagination, setPagination] = useState<PaginationData>();
    const urlQuery = useCurrentQueryParams();
    
    const fetchProducts = () => {
        axios.get('/admin/v1/catalog/products', urlQuery)
            .then((response) => {
    ...
    return (
        <div className="px-4 sm:px-6 lg:px-8">
            <PageHeadingSimple
                ...
                actions={
                    (
                        <Link to='/admin/catalog/products/create'>
                            <button
                                type="button"
                                    ...
                                    {products.map((product) => (
                                        <tr key={product.name}>
                                            ...
                                            <td className="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6 space-x-3">
                                                <Link to={"/admin/catalog/products/details/" + product.id} className="text-indigo-600 hover:text-indigo-900">
                                                    <i className="fa fa-eye text-blue-600"></i> Details<span className="sr-only">, {product.name}</span>
                                                </Link>
                                                <Link to={"/admin/catalog/products/edit/" + product.id} className="text-indigo-600 hover:text-indigo-900">
                                                    <i className="fa fa-edit text-green-600"></i> Edit<span className="sr-only">, {product.name}</span>
                                                </Link>
                                            </td>
                                        </tr>
                                    ))}
